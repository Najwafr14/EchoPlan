...
class CommitteeController extends Controller
{
    public function index(Event $event)
    {
        ...
        $isChief = Auth::user()->isChiefOfCommittee($event->event_id);
    ...
    {
        if (!Auth::user()->isChiefOfCommittee($event->event_id)) {
            return back()->with('error', 'Only Chief of Committee can add members! ❌');
        }
        ...
        $divisionType = DivisionType::find($request->division_type_id);
        $division = Division::firstOrCreate(
            ...
            [
                'division_name' => $divisionType->type_name,
            ]
        );
